...
- The sync report names why a character failed: Public Log off, not found on Bible, blocked by Bible (HTTP 403) or Bible rate limiting. Only other errors still show the raw message in a code span. When every character fails, the reasons are grouped in that order, and the description points to them instead of listing Cloudflare and "Logs not enabled".
- A post in the raid text channel gets one receipt DM instead of one card per raid. The receipt repeats the post as typed (`💬 act4, final, serca solo Qiaoli`), sums up what was written, and shows each character as a `/raid-status` card with the raids from the post and their progress after the write (`🟡 Act 4 Hard · 1/2` for one gate), under `📁` roster headers when the post spans rosters. A raid that was already DONE keeps its row with ℹ️, and a repeated post now reads as nothing new instead of a green "written" card. A reset on a character below the raid's item level shows the raid by name, since `/raid-status` does not list it. Every row names the raid as `/raid-status` does in the reader's language, so a skipped Kazeros row no longer reads "Final Hard" beside "Kazeros Hard".
- Raid gold follows the new patch: Act 4 Normal and Solo pay 10,000 / 17,000 (was 12,500 / 20,500) and Hard 13,500 / 24,500 (was 15,000 / 27,000); Kazeros Normal and Solo 11,000 / 21,000 (was 14,000 / 26,000) and Hard 16,000 / 32,000 (was 17,000 / 35,000); Serca Normal and Solo 13,000 / 19,000 (was 14,000 / 21,000). Normal and Solo still pay half of it bound. Serca Hard and Nightmare and Horizon are unchanged. Gold is computed when shown, so every view, this week's clears included, uses the new amounts.
- `/raid-check` leaves out characters below item level 1720: no card, no "🔒 Chưa đủ điều kiện" line, and none of their raids in the pending counts, the raid and user dropdowns or the roster list. A roster with no character from 1720 is no longer a page, filtered or not, so `/raid-check` opens on the first roster that has one. `/raid-status` still shows every character.
...
- The daily auto-manage run retries later when a character fails with an error other than a disabled Public Log or a name Bible does not know, even if other characters synced, and it moves the last-sync time only when the run fully succeeds. A day with some characters rate-limited used to count as synced, so those characters waited for the next day. A character Bible does not know now settles the day like a private one instead of taking three more attempts.
...
### Added
- The Local Reader preview names the party members registered with Artist that a Full Local Sync would also reach, with the raids and gates they share with your clears. `preview-summary` finds them with the same roster lookup and source-clear checks as the stored preview job, and returns character names only, never their owners.
...
- A Local Sync card narrowed to one roster keeps that roster's name and says Sync still writes every roster. Past eight characters, the closing line says Sync covers the rest and points at the roster filter when the card has one.
- A synced Local Sync card lists what it wrote in the same two-column rows as the preview, read from the current roster. When that roster no longer holds those gates (a weekly reset, a manual reset, a changed difficulty, or a `/raid-status` session opened before the sync), the card keeps the plain gate list.
- `/raid-status` button notices: a button that fails gets its own notice instead of borrowing the Refresh progress one; Refresh progress confirms with one fixed line; New link and Solo Local Reader share one "server not configured" sentence; and the Local Sync entry in the view dropdown says what the view does.
...
- Retrying a Local Sync preview after a write error keeps what the failed attempt wrote. Party members still receive those gates, and the synced card counts them as updated instead of already present.
- A Local Sync apply that fails after writing gates keeps those writes on the preview, as a write error already did. A database error while authorizing party propagation sent the preview back for a retry that found the gates already present, so party members never received them while the card reported a successful sync. A failure in the final write now returns the preview for a retry too, instead of leaving it stuck syncing until the two-minute lease lapsed.
- Sync, Cancel or Refresh in a Local Sync DM no longer puts a line above the card that repeats its result. A DM that already carries that line loses it on its next edit.
- A Local Sync roster filter left from an earlier preview no longer empties the card. `/raid-status` drops the filter when a different preview loads, and a filter that matches no changed roster shows every changed roster again.
...
- A failed whisper confirmation in the raid monitor channel no longer stops the rest of the message handling, so the source message is still cleaned up and the pending hint cleared.
- `/raid-channel config` answers an unknown action with the list of valid actions instead of leaving the interaction to time out.
...
### Fixed
- Bible results arriving after a weekly reset cannot restore the previous week's gate progress. The shared apply path honors the fresh document's reset cursor, including save retries.
- Concurrent MongoDB callers share connection, DNS fallback, and index setup; reconnects no longer accumulate event listeners.
...
- Multi-roster previews get a roster picker plus ◀/▶ buttons and render one roster per page, the way `/raid-status` pages through rosters. Discord forbids mixing a select with buttons in one row, so the arrows sit at the head of the action row and disable at each end. The jobId rides in the customId and the page in the select value, so it needs no session state and behaves identically on `/raid-sync`, on the DM preview, and inside the `/raid-status` sync view.
...
### Changed
- 594 lines of Artist's copy across vi/en/jp now rotate between several phrasings instead of repeating one sentence, spread over 44 pools per language. Covered: raid-status sync results, roster and local-state refresh, the gold-receive toggle, the raid-clear parser's update and reset summaries, the spam warning, the whisper ack, the auto-manage sync report and auto-sync toggles, side-task add/remove, shared-task add/remove, roster-wide task add, schedule kick/add/delete, share grant/revoke, the raid-check refresh and sync titles, and the eleven mention buckets. raid-channel, raid-help, raidBg and auction are left single-string on purpose: they are once-per-setup surfaces where rotating phrasing costs three translations and buys nothing noticeable.
...
### Changed
- `/raid-profile` OVERALL view restyled to a compact ops-brief. Roster + character counts move into the kicker (`// RAID PROFILE · OVERALL · N ROSTER · M CHAR`); `// SCOPE` + `// AGGREGATE` share the top row as two inline fields (short gauge labels Ov/MVP/DPS/SUP); the roster list becomes a fenced **code-block table** with aligned columns, ranked best-first (`# ROSTER NAME CHAR LOG SCORE`). Numeric score only - a per-row gauge pushes the line past the ~42-col embed code-block wrap point and detaches the score onto the next line. For density, the 2-line description, the per-roster Own/Shared tag, and the separate ★ MVP-top line are dropped (Overall-top folded into the SCOPE `★ top` row, now prefixed with the character's class icon + weapon-emoji fallback). Locale unchanged. 852 tests green.
...
- Follow-up: the lead **Manage menu**, **End summary**, and **Room reveal** got the fuller treatment too - the Manage menu now shows a mono `raid · STATUS` line + a slot-fill gauge + waitlist count (a glanceable console for the lead), End summary leads with a clears gauge (`▰▰▰▱ updated/targets`), and Room reveal carries a mono raid kicker line. `STATUS_CODE` exported from board.js for reuse.
- Dropdown consistency: the **Kick** and **Phân turn member-select** dropdowns now show the class as the option icon (not a class-name in the description) with an `account · ilvl · role` line, matching the Join / Add-member pickers - so every character dropdown in raid-schedule looks identical.
...
- Locale strings (vi/en/jp, parity), `/raid-help`, and README synced in the same change. Routing needed no registry change (Kick/kickpick fall under the existing `rse:` button + select prefixes). 514 tests green across two runs (+3 applyKick unit tests, +1 turn-cleanup regression).

### Fixed
- Kicking a member now also removes that Discord id from every saved turn assignment, so the Phân turn panel and stored show-plan data do not keep stale kicked members.
...
### Added
- `/raid-auction` slash command: an auction bid calculator for shared raid loot. Options: `players` (choice 4 / 8), `market_value` (positive integer, gold), and optional `profit` (boolean, default on). Replies publicly (the auction happens live in the party, so the whole channel should see the suggested bid) with the recommended bid (raw integer in a code block for copy-paste into the in-game auction box), what each of the other `N-1` members receives, and the winner's estimated profit vs market. Only the invalid-input notice stays ephemeral.
