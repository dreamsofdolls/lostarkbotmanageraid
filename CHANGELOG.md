...
- **Railway `startCommand` decoupled from schema registration success.** Changed the chain operator from `&&` to `;` so a deploy-commands failure (rate limit, Discord outage, bad token, script hang) no longer blocks `node src/bot.js`. Users keep existing commands and a stale-schema annoyance is strictly better than the bot being offline. `deploy-commands.js` still exits 1 on error so the failure remains visible in Railway logs. `railway.toml`.
- **[BUG FIX] `deploy-commands.js` now explicitly exits with code 0 after a successful registration.** The discord.js REST client keeps a keep-alive HTTP agent alive internally, so natural event-loop drain never happens after `rest.put(...)` completes. Without an explicit `process.exit(0)`, the script hangs forever, which broke the Railway `&& node src/bot.js` chain — the bot never started, MongoDB never connected, and the bot appeared offline. Added `process.exit(0)` on the success path of `deploy-commands.js`. `src/deploy-commands.js`.
- **Railway now auto-registers slash commands on every boot.** `railway.toml` `startCommand` changed from `node src/bot.js` to `node src/deploy-commands.js && node src/bot.js`. Every redeploy / restart re-runs `deploy-commands.js` against Discord before booting the bot, so schema on Discord always matches the deployed code. If registration fails (bad token, Discord API down), the bot does not start — the failure surfaces in Railway logs instead of silently leaving stale commands. No more "deploy code but /raid-set options still show old labels" confusion. `railway.toml`.
