...
- New `/raid-channel config action:set-bg-channel channel:#<name>` admin action stores the rehost target on `GuildConfig.raidBgChannelId`. Sits next to the existing `/raid-channel config action:set` (monitor channel) flow so admins discover both knobs under the same command instead of digging through Railway env vars. Empty/unset = `/raid-bg set` rejects with a clear "ask admin" alert; `/raid-status` falls back to the text embed for everyone, matching pre-feature behavior exactly.
