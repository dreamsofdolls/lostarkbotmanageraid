...
function summarizeGroup(rows) {
  ...
  return {
    ...
    avgRdpsDamageGivenShare: round1(average(rdpsRows.map((r) => r.rdpsDamageGivenShare))),
...
function computeBuildStats(profileRows, role, buildVariants) {
  ...
  return {
    ...
    avgSynergyGivenShare: round1(average(profileRows.map((r) => r.synergyGivenShare))),
  ...
  return (rows || []).slice(0, MAX_PROFILE_ENCOUNTER_SUMMARIES).map((row) => {
    ...
    return {
      ...
      metrics: {
        ...
        supportAp: round2(row.supportAp),
        supportBrand: round2(row.supportBrand),
        supportIdentity: round2(row.supportIdentity),
        supportHyper: round2(row.supportHyper),
        ...
        rdpsDamageGivenShare: round1(row.rdpsDamageGivenShare),
        ...
        synergyGivenShare: round1(row.synergyGivenShare),
