...
module.exports = {
  ...
  "local-sync-discord": {
    ...
    stateDescriptions: {
      ...
      superseded: "Một lần quét mới hơn đã thay thế preview này. Mở `/raid-sync` để xem bản mới nhất.",
    ...
    jobMissing: "Preview này không còn tồn tại hoặc đã được dọn. Mở `/raid-sync` để quét lại nhé.",
  ...
  "stuck-nudge": {
    ...
    switchedDescription: "<@{target}> đã chuyển từ bible auto-sync sang **local-sync mode**. Mở `/raid-sync` để vào Local Sync Console hoặc lấy link Local Reader mới nha~",
    dmTitle: "Local Sync setup",
    dmDescription: "Bấm **Mở Local Reader** để chọn `encounters.db` ở `%localappdata%\\LOA Logs\\`. Reader nhớ file cho lần sau và chỉ gửi preview nhỏ về Discord; cậu xác nhận bằng `/raid-sync`. Link có hiệu lực 30 phút.",
  },
  ...
  "raid-auto-manage": {
    ...
    localEnable: {
      ...
      successDescription: "Local-sync mode đã bật, nhưng Artist chưa tạo được link Local Reader. Nhờ Manager kiểm tra cấu hình và log, rồi mở `/raid-sync` để thử lấy link mới. Cậu vẫn có thể dùng `/raid-set` thủ công. Đổi ý thì `action:local-off`.",
      successDescriptionWithLink: "Local-sync mode đã bật xong nha~ Bấm **Mở Local Reader**, chọn `encounters.db` lần đầu rồi gửi preview sang Discord. Sau đó dùng `/raid-sync` để kiểm tra và xác nhận. Reader nhớ file; link có hiệu lực 30 phút và có thể làm mới từ `/raid-status`. Đổi ý thì `action:local-off`.",
    ...
    sync: {
      ...
      localLockedDescription: "Manual `action:sync` là sync từ lostark.bible, nên tớ chặn khi **local-sync** đang bật. Mở `/raid-sync` để dùng Local Reader/Discord Console hoặc chạy `/raid-auto-manage action:local-off` nếu muốn chuyển lại bible.",
      failTitle: "Sync fail",
  ...
  "raid-help": {
    ...
    sections: {
      ...
      "raid-sync": {
        label: "/raid-sync",
        short: "Mở Local Sync Console riêng tư để quét, preview và xác nhận tiến độ",
        example: "/raid-sync",
        notes: [
          "Mở console Local Sync không dùng collector dài hạn. Preview được lưu ở server nên bot restart hoặc session `/raid-status` hết hạn vẫn mở lại được.",
          "",
          "**Lần đầu**: bấm `Mở Local Reader`, chọn `encounters.db`; trình duyệt nhớ quyền đọc cho lần sau.",
      ...
      "raid-auto-manage": {
        ...
        notes: [
          ...
          "**`action:local-on` flow**: mint URL signed-token (TTL 30 phút) và trả trong reply riêng tư. Mở Local Reader → chọn file lần đầu → gửi preview → kiểm tra char/raid/gate và bấm Đồng bộ trong `/raid-sync`. Preview lưu 2 giờ; scan mới thay preview pending cũ.",
