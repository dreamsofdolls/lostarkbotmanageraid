...
module.exports = {
  ...
  "raid-schedule": {
    ...
    notice: {
      ...
      resurfacedTitle: "Đã đẩy board xuống đáy~",
  ...
  "raid-status": {
    ...
    taskView: {
      ...
      viewToggleGoldLabel: "Gold nhận",
      viewToggleGoldDescription: "Xem + toggle raid nào tính gold cho từng character",
    ...
    goldView: {
      embedTitle: "💰 Gold nhận · {accountName}",
      ...
      modeAppliedTitle: "Đã đổi độ khó",
      ...
      modeDeferredTitle: "Đã hẹn đổi độ khó",
      ...
      modeCancelledTitle: "Đã huỷ đổi độ khó",
  ...
  "raid-check": {
    ...
    refreshFlow: {
      successTitle: "Đã refresh roster",
      ...
      noUpdateTitle: "Đã thử refresh roster",
  ...
  raidProfile: {
    overallTitle: "Hồ sơ raid của cậu",
    overallDesc:
      "Artist gom từ `encounters.db` của cậu.\nChỉ đếm boss raid Artist hỗ trợ, đã clear, trận dài hơn 3 phút.",
    rosterTitle: "Hồ sơ · {account}",
    rosterOwn: "Roster của cậu.",
    rosterShared: "Roster này **{owner}** share cho cậu (quyền {level}).",
    updatedAt: "Cập nhật {date}",
    confidence: "Độ tin cậy: **{conf}** ({n} log)",
    author: {
      overall: "// RAID PROFILE · TỔNG QUAN · {rosters} roster · {characters} nhân vật",
      roster: "// RAID PROFILE · ROSTER · {account} · {characters} nhân vật · {ownership}",
      character: "// RAID PROFILE · NHÂN VẬT · {character} · {role}",
      explain: "// RAID PROFILE · GIẢI THÍCH",
    },
    explain: {
      title: "Giải thích chỉ số đang xem",
      names: {
        roleAvg: "DPS avg / Support avg",
      },
      defs: {
        overallScore: "Điểm tổng hợp 0-100 gộp mọi mặt (sát thương/đóng góp, sống sót, ổn định), càng cao càng tốt.",
        mvpScore: "Khả năng char là người chơi hay nhất trận, so đóng góp của char với cả party.",
        roleAvg: "Điểm trung bình của các char cùng vai (DPS hoặc Support) trong phạm vi đang xem.",
        survival: "Mức sống sót: thưởng cho ít chết và ít thời gian nằm đất.",
        stability: "Độ ổn định giữa các trận: điểm dao động càng ít thì càng cao.",
        contextPct: "Percentile phong độ so với người cùng class và cùng raid trên cộng đồng (70% = giỏi hơn 70% số đó).",
        damageShare: "Phần trăm sát thương char đóng góp trong tổng damage của party.",
        peakBurst: "DPS đỉnh trong 10 giây kèm hệ số bùng nổ (đỉnh so với DPS trung bình).",
        deathless: "Phần trăm số trận char không chết lần nào.",
        rdpsImpact: "Điểm tác động hỗ trợ 0-100: char support nâng sức mạnh cả party tới đâu (rDPS cộng thêm và uptime buff).",
        contribution: "Phần synergy (buff tăng sát thương) char cộng cho party, tính theo phần trăm tổng synergy của party.",
        rContribution: "Phần rDPS (DPS thật cộng thêm) char support tạo ra, tính theo phần trăm tổng của party.",
        supporterPct: "Phần trăm đóng góp hỗ trợ của support theo cách LoaLogs tính (gộp uptime buff/debuff).",
        radiantPct: "Phần trăm số trận char support đạt mức 'radiant', ngưỡng chất lượng buff cao.",
        supportRank: "Thứ hạng trung bình giữa các support trong cùng trận (1 là tốt nhất).",
        protection: "Lượng che chắn (shield và giảm sát thương) char cấp cho party mỗi phút.",
        apBrand: "Uptime của Attack Power buff và Brand debuff, hai thứ chủ lực của support.",
        identityHyper: "Uptime kỹ năng Identity (đặc thù class) và Hyper, mức tận dụng skill mạnh.",
      },
    },
    sections: {
      scope: "Phạm vi",
    ...
    footer: {
      ...
      snapshotMissing: "Snapshot chưa có",
      logCount: "{logs} log",
      scoredCount: "{scored} log chấm điểm",
      confidence: "Độ tin cậy {confidence}",
    },
    lines: {
      logScored: "Log / chấm điểm: **{logs} / {scored}**",
      scoredLogs: "**{logs}** log chấm điểm",
      topCharacter: "★ nổi bật: {icon} **{name}** {score}",
      topMissing: "★ nổi bật: **N/A**",
      confidence: "Độ tin cậy **{confidence}**",
    },
    labels: {
      dps: "DPS",
    ...
    noRosterTitle: "Artist chưa thấy roster nào của cậu",
    noRosterDesc:
      "Cậu `/raid-add-roster` trước, hoặc nhờ manager share roster cho cậu. Sau đó mở Web Companion cho Artist đọc `encounters.db`, Artist sẽ dựng hồ sơ raid cho cậu.",
    noSnapshotTitle: "Hồ sơ còn trống",
    noSnapshotHintOn:
      "Local-sync đang bật. Cậu mở Web Companion từ `/raid-status`, chọn hoặc restore `encounters.db`, đợi dòng profile auto-sync báo xong, rồi gọi lại `/raid-profile` là Artist gom hồ sơ cho cậu.",
    noSnapshotHintOff:
      "Cậu bật `/raid-auto-manage action:local-on`, mở Web Companion, rồi chọn `encounters.db` để tạo profile snapshot trước.",
    noSnapshotHintBible:
      "Bible auto-sync đang bật, nhưng `/raid-profile` cần chỉ số encounter sâu hơn phần Bible progress sync hiện đang lưu. Muốn dựng hồ sơ đầy đủ thì cậu tắt Bible sync bằng `/raid-auto-manage action:off`, rồi bật local-sync bằng `action:local-on` và chọn `encounters.db` trong Web Companion nha.",
    forbiddenTitle: "Hồ sơ này không phải của cậu",
    forbiddenDesc: "Nút này thuộc phiên `/raid-profile` của người khác.",
    expiredTitle: "Phiên xem hồ sơ đã hết hạn",
    expiredDesc: "Cậu gõ lại `/raid-profile` để Artist mở phiên mới.",
    optOverall: "Tổng quan",
    optOverallDesc: "Gộp hết roster cậu xem được",
    rosterPlaceholder: "Chọn roster để xem",
    optRosterOverview: "Tổng quan roster",
    optRosterOverviewDesc: "Xem tổng hợp cả roster",
    optPickRosterFirst: "Chọn roster trước",
    optPickRosterFirstDesc: "Chọn roster ở trên, char sẽ hiện ra đây.",
    charPlaceholder: "Chọn character",
    optCharacterDesc: "{logs} log · điểm {score}",
    btnPrev: "Trước",
    btnNext: "Sau",
