...
const HELP_SECTIONS = [
    ...
    },
    {
      ...
      notes: [
        ...
        "EN: **💰 Gold tracking**: gold-earner chars carry a `· 💰` suffix in the header right after the iLvl, plus a body line `💰 earned / total G` (unbound). Per-account rollup line in description sums every gold-earner. LA caps at 6 gold-earners/account/week - mark them via `/raid-gold-earner roster:<name>`. Non-earner chars are silent (no body line) - the absence of the header 💰 is the only signal. Raid filter narrows the gold sums in lockstep with the raid counts.",
        "VN: **💰 Track gold**: char gold-earner có suffix `· 💰` trên header ngay sau iLvl, kèm dòng body `💰 đã kiếm / tổng G` (unbound). Dòng rollup trong description cộng dồn mọi gold-earner. LA cap 6 gold-earner/account/tuần - mark bằng `/raid-gold-earner roster:<tên>`. Char không phải gold-earner thì im (không hiện body line) - absence của 💰 trên header là tín hiệu duy nhất. Raid filter narrow gold theo raid đó luôn.",
    ...
    {
      key: "raid-gold-earner",
      label: "/raid-gold-earner",
      icon: "💰",
      short: "Pick which 6 chars per roster earn weekly gold",
      shortVn: "Chọn 6 char nhận gold mỗi tuần (cap LA)",
      options: [
        { name: "roster", required: true, desc: "Roster cần config / Roster (account) to configure (autocomplete)" },
      ],
      example: "/raid-gold-earner roster:Qiylyn",
      notes: [
        "EN: Opens an ephemeral picker (5-min session) to flip the `💰 isGoldEarner` flag per character. Lost Ark caps at 6 gold-earners per account per week, so the picker hard-stops at 6 ticks.",
        "VN: Mở picker ephemeral (5 phút) để flip flag `💰 isGoldEarner` cho từng char. LA cap 6 gold-earner / account / tuần nên picker chặn ở mức 6 tick.",
        "",
        "EN: **First-open UX**: if every char in the account is currently non-earner (legacy data from before 2026-05-05), Artist pre-checks the top 6 by iLvl as a one-click migration. You can untick + retick freely before clicking **Confirm**.",
        "VN: **Lần đầu mở**: nếu cả account hiện không có ai là gold-earner (data cũ trước 2026-05-05), Artist tự pre-check top 6 char theo iLvl để migration 1 click. Cậu có thể untick + retick tuỳ ý trước khi bấm **Confirm**.",
        "EN: **Default for new chars**: chars added via `/add-roster` after 2026-05-05 default to gold-earner ON. Use this command to flip them off if you have alts beyond the in-game pick.",
        "VN: **Mặc định cho char mới**: char add qua `/add-roster` sau 2026-05-05 default là gold-earner. Dùng lệnh này để flip OFF nếu cậu có alt ngoài top 6 in-game.",
        "EN: **Cap behavior**: ticking a 7th char surfaces an ephemeral notice and rejects the change - untick someone else first.",
        "VN: **Cap behavior**: tick char thứ 7 sẽ ra notice ephemeral và reject - untick char khác trước rồi mới tick được.",
        "EN: **Off-window chars**: rosters with > 20 chars (rare) only show the 20 highest iLvl chars in the picker. The off-window 21st+ chars keep their existing flag value untouched.",
        "VN: **Char ngoài cap picker**: roster > 20 char (hiếm) chỉ show 20 char iLvl cao nhất ở picker. Char thứ 21+ giữ nguyên flag cũ, không bị đụng.",
      ],
    },
    {
      key: "raid-task",
      ...
      notes: [
        ...
        "EN: **NA West schedule presets**: Chaos Gate = Mon/Thu/Sat/Sun hourly 11 AM-5 AM PT · Field Boss = Tue/Fri/Sun hourly 11 AM-5 AM PT.",
        "VN: **Preset lịch NA West**: Chaos Gate = T2/T5/T7/CN mỗi giờ 11 AM-5 AM PT · Field Boss = T3/T6/CN mỗi giờ 11 AM-5 AM PT.",
