...
const HELP_SECTIONS = [
    ...
    },
    {
      ...
      notes: [
        ...
        "• **Freshness + countdown line (real-time tick)**: mỗi account embed có dòng `📥 Last updated <t:R> · ⏳ Refresh ready <t:R> · 🔄 Last synced <t:R> · ⏳ Sync ready <t:R>` ở description. Discord native timestamp `<t:UNIX:R>` browser tự tick mỗi giây, không cần server re-render. Wording `Refresh ready` / `Sync ready` cố ý cho cả 2 tense đọc OK: future `Sync ready in 14s` + past `Sync ready 16s ago` (= đã ready 16s rồi). Khi render-time cooldown đã hết → `✅ Refresh ready` / `✅ Sync ready` (không timestamp). `📥` = roster metadata (iLvl/class) gate 2h. `🔄` = auto-manage bible log sync gate **per-user** (`15s` cho Raid Manager, `10 phút` cho user thường) - chỉ hiện khi user đã opt-in.",
        ...
        "• **View toggle dropdown** (`📋 Tiến độ raid` ↔ `📝 Side tasks`): row dưới raid-filter, chuyển embed body giữa 2 view. Task view show daily/weekly chores đã đăng ký qua `/raid-task` per character (cap 3 daily + 5 weekly), kèm dropdown `Bấm để toggle complete...` để tick/untick mà không cần command thêm. Pagination preserve giữa 2 view - đang ở account 2 toggle sang Task view vẫn ở account 2. Auto-reset hằng ngày 17:00 VN (daily) + thứ 4 17:00 VN (weekly).",
      ],
    },
    {
      key: "raid-task",
      ...
      shortVn: "Đăng ký task daily/weekly cho từng character",
      options: [
        { name: "subcommand", required: true, desc: "`add` · `remove` · `clear`" },
        { name: "action", required: false, desc: "(add) `single` = một char cụ thể · `all` = mọi char trong roster" },
        { name: "roster", required: true, desc: "Roster (account) chứa character (autocomplete)" },
        { name: "character", required: false, desc: "(add action=single / remove / clear) Character muốn gắn/xoá task (autocomplete filter theo roster)" },
        { name: "name", required: false, desc: "(add) Tên task, autocomplete gợi ý từ task cũ của cậu, max 60 ký tự" },
        { name: "reset", required: false, desc: "(add) `daily` (reset 17:00 VN mỗi ngày) hoặc `weekly` (reset 17:00 VN thứ 4)" },
        { name: "task", required: false, desc: "(remove) Task cần xoá, autocomplete list theo character đã pick" },
      ],
      example: "/raid-task add action:single roster:Clauseduk character:Frostmourne name:Una Dailies reset:daily",
      notes: [
        "EN: Lightweight chore tracker scoped to a single character. Tasks live next to the character so each char gets its own checklist - one char's Una dailies don't bleed into another char's view.",
        "VN: Tracker task phụ siêu nhẹ gắn từng character riêng. Task của char A không hiện ở char B - mỗi char tự giữ checklist của mình.",
        "• **Cap cứng**: 3 daily + 5 weekly mỗi character. Vượt cap → embed warn `Đã đầy slot · Slot còn lại 0/3 daily ...`. Đặt cap để view không bị loãng + giữ Discord embed gọn.",
        "• **Action `single` vs `all`**: `single` add cho 1 char (cần field `character`). `all` add cùng task cho **mọi char trong roster** (bỏ qua field `character`). Mỗi char check cap + dup riêng khi all-mode - char đầy cap hoặc đã có task được skip kèm tên trong summary `Added N · Skipped dup K · Skipped cap L`. Tiết kiệm 6 lần gõ khi chore áp cho mọi alt.",
        "• **Name autocomplete**: gõ field `name:` Artist gợi ý từ những task cậu đã từng tạo (dedup theo name + reset, sort recency). Đỡ nhớ và đỡ gõ trùng tên ở nhiều char.",
        "• **Auto-reset bằng scheduler riêng** (30-min tick, độc lập với `AUTO_MANAGE_DAILY_DISABLED` killswitch): khi cycle boundary qua, scheduler bulk `updateMany` flip `completed=false` cho mọi task expire. Daily boundary = 17:00 VN (10:00 UTC) mỗi ngày. Weekly boundary = thứ 4 17:00 VN. Steady state khi không task nào expire = no-op (pre-filter `$elemMatch` short-circuit).",
        "• **Toggle complete trong `/raid-status` Task view**: gõ `/raid-status` → bấm dropdown `📝 Side tasks` → chọn character (hoặc `🌐 Tất cả character` để bulk) → bấm dropdown `Toggle task...` để flip 🟢/⚪. Không cần command riêng.",
        "• **Bulk toggle**: chọn `🌐 Tất cả character` ở dropdown char-filter → toggle dropdown gom mọi (name, reset) task aggregated. Click 1 task → flip cùng state cho mọi char đang sở hữu task đó (mọi char `🟢` → tất cả về `⚪`, ngược lại tất cả về `🟢`). Use case: vừa xong Paradise daily cho 6 alts, click 1 lần xong cả 6.",
        "• **Mỗi người có list riêng**: side tasks gắn với character của bản thân, hiển thị ở `/raid-status` của chính mình. Mọi user đều có cùng feature, mỗi người tự quản lý checklist của mình.",
        "• **Subcommand `clear`**: pop ephemeral confirm button trước khi xoá toàn bộ - tránh accident xoá nhầm. Chỉ caller bấm được. Bấm `Huỷ` thì không xoá gì.",
        "• **Duplicate guard**: cùng `name` + `reset` trên cùng character → reject (`Task đã tồn tại`). Cho phép cùng name khác cycle (vd `Una` daily + `Una` weekly OK).",
      ],
    },
    {
      key: "raid-set",
    ...
    },
    {
      ...
      notes: [
        ...
        "• **2 rosters per page (chunked)**: mỗi embed page chứa tối đa 2 roster sections stacked. Roster section = non-inline header field với explicit value line. Name = `📁 accountName (displayName)` (clean label). Value = state breakdown trên dòng 1, refresh + sync trên 2 dòng riêng biệt phía dưới. **📥 Last updated** (dòng riêng) = roster data refresh (iLvl/class từ bible qua `/raid-status` lazy refresh HOẶC pre-scan refresh của `/raid-check`) - applies to ALL users, 2h cooldown. **🔄 Last synced** (dòng riêng dưới refresh) = auto-manage bible log sync (raid progress) - chỉ opted-in user, `Never synced` nếu chưa có data, cooldown **per-user** (`15s` cho Raid Manager, `10 phút` cho user thường). **⏳ Refresh/Sync ready countdown** pairs với mỗi badge dùng Discord native timestamp `<t:UNIX:R>` real-time tick browser-side - khi cooldown còn active show countdown future, khi expired show `✅ Refresh ready` / `✅ Sync ready` để user không phải đoán mò tại sao Sync button bấm xong data không đổi. Ví dụ:\n  `4 ⚪ · 1 🟡 · 1 🟢`\n  `📥 Last updated <t:R> · ⏳ Refresh ready <t:R>`\n  `🔄 Last synced <t:R> · ⏳ Sync ready <t:R>`\nPer roster cost: 1 header + N char + ceil(N/2) spacer fields. 2 × 6-char rosters = 20 fields, fit 25-cap.",
        "• **Manager Task view toggle (`📝 Xem tasks` ↔ `📋 Quay lại raid scan`)** trong `raid:all`: hiện khi user filter narrow xuống 1 user. Click button → swap embed in-place từ raid scan sang Task view (per-account char layout giống `/raid-status` Task view), không spawn followup riêng. Edit + Bật/Tắt auto-sync + raid-filter dropdown ẩn trong Task view (focused mode). Click button lần nữa quay về raid scan. Page index preserve giữa 2 view nên Manager đang xem account #2 toggle qua lại vẫn ở account #2. Read-only - không có toggle dropdown nên Manager không sửa data member.",
        ...
        "• **Sync badge trong roster header**: opted-in user có sync data hiện `🔄 Last synced <t:R>`. Opted-in nhưng chưa sync lần nào → `🔄 Never synced`. Non-opted-in → không hiện segment này. Kèm countdown `⏳ Sync ready <t:R>` (cooldown gate bởi `lastAutoManageAttemptAt`, render-time cooldown active) hoặc `✅ Sync ready` khi cooldown expired ngay từ render. Cooldown per-user: Raid Manager (env `RAID_MANAGER_ID`) = **15s**, everyone else = **10 phút** - 2 người operators reconcile nhanh sau clear, member thường không flood bible.lostark.",
    ...
    },
    {
      ...
      options: [
        { name: "type", required: true, desc: "Loại announcement - dropdown 9 giá trị: `weekly-reset` / `stuck-nudge` / `set-greeting` / `hourly-cleanup` / `artist-bedtime` / `artist-wakeup` / `whisper-ack` / `maintenance-early` / `maintenance-countdown`. 4 loại CHANNEL_OVERRIDABLE (`weekly-reset`, `stuck-nudge`, `maintenance-early`, `maintenance-countdown`) chấp nhận `set-channel`; 5 loại còn lại CHANNEL_BOUND (chỉ toggle on/off được)." },
      ...
      example: "/raid-announce type:maintenance-early action:set-channel channel:#raid-announcements",
      notes: [
        "• **Override resolution note**: 4 type overridable (`weekly-reset`, `stuck-nudge`, `maintenance-early`, `maintenance-countdown`) resolve đích qua `announcements.<type>.channelId || raidChannelId`, nên `/raid-announce ... action:set-channel` vẫn chạy kể cả khi monitor channel chưa set.",
        "EN: Manage Artist's 9 channel-announcement surfaces per guild. Two axes: enabled toggle + channel override (overrideable types only).",
        "VN: Quản lý 9 loại announcement Artist đang post vào channel, per-guild. 2 trục config: enabled + channel override.",
        "• **9 announcement types**: `weekly-reset` (Wed 17 VN tuần mới), `stuck-nudge` (phase 3 tick tag user toàn char private log), `set-greeting` (greeting ephemeral sau /raid-channel action:set), `hourly-cleanup` (notice sau cleanup mỗi slot 30 phút ban ngày), `artist-bedtime` (3h sáng VN Artist đi ngủ), `artist-wakeup` (8h sáng VN Artist dậy + catch-up sweep), `whisper-ack` (tag user trong channel khi /raid-channel post clear DM success), `maintenance-early` (3 mốc T-3h/T-2h/T-1h trước bảo trì Wed 14:00 VN, ping `@here` ở T-3h và T-1h), `maintenance-countdown` (4 mốc T-15m/T-10m/T-5m/T-1m không ping).",
        "• **Channel-overridable vs channel-bound**: `weekly-reset` + `stuck-nudge` + 2 maintenance type là pure announcements có thể redirect sang #announcements / #maintenance riêng. 5 loại còn lại (`set-greeting` / `hourly-cleanup` / `artist-bedtime` / `artist-wakeup` / `whisper-ack`) bound với monitor channel vì content refer cụ thể tới channel đó (\"channel này vừa dọn xong\" / \"Artist được mời đến channel này\" / bedtime-wakeup đều nói về channel này / whisper reply tin gốc) - `set-channel`/`clear-channel` sẽ reject với 5 loại bound này.",
        "• **Maintenance reminder schedule**: hard-coded LA VN bảo trì Wed 14:00 VN. 7 mốc fire qua scheduler 1-min tick: early group (T-3h/T-2h/T-1h, TTL 30 phút, ping @here ở T-3h+T-1h) + countdown group (T-15m/T-10m/T-5m/T-1m, TTL 10 phút trừ T-1m TTL 5 phút, không ping). Mỗi mốc random pick 1 trong 3 variants. Dedup per group qua `lastMaintenanceEarlyKey` / `lastMaintenanceCountdownKey` = `YYYY-MM-DD:slotKey`. Bot offline qua mốc → mốc đó skip (chấp nhận, mốc sau vẫn fire). Bật/tắt 2 group độc lập: server có thể giữ 3 reminder đầu mà tắt 4 countdown, hoặc ngược lại.",
        ...
        "• **Mongo path**: `GuildConfig.announcements.<subdocKey>.enabled|channelId`. Subdoc key map: `weekly-reset`→`weeklyReset`, `stuck-nudge`→`stuckPrivateLogNudge`, `set-greeting`→`setGreeting`, `hourly-cleanup`→`hourlyCleanupNotice`, `artist-bedtime`→`artistBedtime`, `artist-wakeup`→`artistWakeup`, `whisper-ack`→`whisperAck`, `maintenance-early`→`maintenanceEarly`, `maintenance-countdown`→`maintenanceCountdown`. Keys sống trong central `ANNOUNCEMENT_REGISTRY` object (raid/announcements.js) - single source of truth cho label + subdocKey + channelOverridable flag. Adding a new type: 1 registry entry + 1 schema subdoc + firing site code (không cần edit command builder hay helper vì tất cả derived).",
