...
const HELP_SECTIONS = [
    ...
    },
    {
      ...
      notes: [
        ...
        "• **Manager Task view button (`📝 Xem tasks`)** trong `raid:all`: hiện bên cạnh `Bật/Tắt auto-sync` khi user filter narrow xuống 1 user cụ thể. Click → ephemeral followup hiển thị side tasks của user đó (per-account, 2-column char layout giống `/raid-status` Task view), read-only - không có toggle dropdown nên Manager không sửa được data của user khác. Dùng để Manager monitor chore progress mà không phải ping/DM member.",
