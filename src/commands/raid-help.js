...
const HELP_SECTIONS = [
    ...
    },
    {
      ...
      notes: [
        ...
        "• **Per-user sync throttle**: cooldown + in-flight guard. Cooldown **per-user**: Raid Manager (env `RAID_MANAGER_ID`) = 30s, mọi user khác = 15 phút - operators reconcile nhanh sau clear, member thường không spam bible. `action:sync` spam → reject ephemeral với remaining time. `action:on` đang in-flight thì reject; đang cooldown thì vẫn flip flag nhưng skip cả probe lẫn sync, báo user chờ X phút/giây rồi gõ `sync` sau.",
