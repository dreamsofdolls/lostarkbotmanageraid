...
const HELP_SECTIONS = [
    ...
    },
    {
      ...
      options: [
        { name: "subcommand", required: true, desc: "`add` · `add-all` · `remove` · `clear`" },
        { name: "roster", required: true, desc: "Roster (account) chứa character (autocomplete)" },
        { name: "character", required: false, desc: "(add/remove/clear) Character muốn gắn/xoá task (autocomplete filter theo roster)" },
        { name: "name", required: false, desc: "(add/add-all) Tên task, autocomplete gợi ý từ task cũ của cậu, max 60 ký tự" },
        { name: "reset", required: false, desc: "(add/add-all) `daily` (reset 17:00 VN mỗi ngày) hoặc `weekly` (reset 17:00 VN thứ 4)" },
        { name: "task", required: false, desc: "(remove) Task cần xoá, autocomplete list theo character đã pick" },
      ],
      example: "/raid-task add roster:Clauseduk character:Frostmourne name:Una Dailies reset:daily",
      notes: [
        ...
        "• **Subcommand `add-all`**: thêm cùng task cho **mọi character** trong 1 roster. Mỗi char check cap + dup riêng - char đầy cap hoặc đã có task này được skip kèm tên trong summary, char còn lại được add. Embed reply tách rõ `Added N · Skipped dup K · Skipped cap L`. Tiết kiệm 6 lần gõ lệnh khi chore áp cho mọi alt.",
        ...
        "• **Toggle complete trong `/raid-status` Task view**: gõ `/raid-status` → bấm dropdown `📝 Side tasks` → bấm dropdown `Toggle task của <char>...` để flip 🟢/⚪. Không cần command riêng - command `/raid-task` chỉ để add/add-all/remove/clear schema, toggle complete là UI.",
