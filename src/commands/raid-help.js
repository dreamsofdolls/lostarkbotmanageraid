...
const HELP_SECTIONS = [
    ...
    },
    {
      ...
      notes: [
        ...
        "• **Sync ngay button** (chỉ hiện cho opted-in user): nút `🔄 Sync ngay` ở dưới embed - bấm 1 cái là Artist re-pull bible + apply ngay, embed update tại chỗ không cần re-issue lệnh. Cùng cooldown gate với piggyback (`15s` Manager, `10 phút` user thường). Cooldown chưa hết → ephemeral báo `\"đợi X phút nữa nha\"` thay vì silent skip. Mục đích: user đang xem embed muốn refresh không phải close + chạy lại `/raid-status`.",
