...
const HELP_SECTIONS = [
    ...
    },
    {
      ...
      options: [
        ...
        { name: "task", required: false, desc: "(remove) task cần xoá - autocomplete theo char / Task to remove (autocomplete by character)" },
      ],
    ...
    },
    {
      ...
      options: [
        { name: "raid", required: true, desc: "Raid + difficulty cần scan (hoặc `all` xem tổng) / Raid + difficulty to scan (or `all` for cross-raid overview)" },
      ],
      example: "/raid-check raid:kazeros_hard",
      notes: [
        "EN: Scans every character in the server still pending the selected raid. Restricted to Raid Managers (env `RAID_MANAGER_ID`).",
        ...
        "EN: **Filter**: user / raid dropdown to zoom. Picking 1 user reveals `Edit progress` / `Bật/Tắt auto-sync hộ` buttons. In `raid:all` the `📝 Xem tasks` toggle also appears for chore monitoring.",
        "VN: **Filter**: dropdown user hoặc raid để zoom. Pick 1 user → button `Edit progress` / `Bật/Tắt auto-sync hộ` xuất hiện. Trong `raid:all` còn có thêm `📝 Xem tasks` để monitor chore.",
        ...
        "EN: **`raid:all` (cross-raid overview)**: paginates per user/account. Toggle `📝 Xem tasks` to view a member's chores (read-only).",
        "VN: **`raid:all` (cross-raid overview)**: pagination per user/account. Toggle `📝 Xem tasks` để xem chore của member (read-only).",
