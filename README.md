...
### `/raid-auto-manage` (Phase 2 — auto-sync via `/raid-status` piggyback)

Kéo clear logs từ `lostark.bible/api/character/logs` và reconcile tự động vào `assignedRaids`. **Phase 2 (live)**: khi `User.autoManageEnabled = true`, mỗi lần user gõ `/raid-status` sẽ trigger 1 lần auto-sync gated bởi cooldown 5 phút + in-flight guard (xem `/raid-status` Lazy auto-refresh ở trên). User vẫn có thể trigger manual qua `action:sync` bất cứ lúc nào.
...
- **Phase 2 auto-sync**: `handleStatusCommand` piggybacks auto-manage sync vào lazy-refresh flow. Khi `autoManageEnabled = true` và `acquireAutoManageSyncSlot` allow (cooldown hết + không in-flight), `/raid-status` gather bible logs trong cùng collect pass với roster refresh và apply vào cùng fresh doc trong 1 save — atomic. User opt-in chỉ cần `/raid-status` để vừa xem progress vừa auto-sync, không còn phải gõ `action:sync` riêng. Gather fail (Cloudflare/timeout) được swallow + log + render cached → `/raid-status` không bao giờ bị vỡ vì auto-sync.
