...
- **Per-roster section header (rich value)**: non-inline field với name = `📁 <accountName> (<displayName>)` (clean label, không wrap), value = RICH stats line `<state breakdown> · avg iLvl <N> · 🔄<relative>` (ví dụ `4 ⚪ · 1 🟡 · 1 🟢 · avg iLvl 1704 · 🔄1h`). **State breakdown** qua `formatRosterStats()` - count chars theo từng state (filter zero), giúp Raid Manager scan nhanh "roster này progress hay chưa chạm". **Avg iLvl** compute từ pending chars của roster - "do I expect these chars to clear?" insight. **Sync badge** ở cuối nếu opted-in. Rich content fill value line → Discord inter-row padding look proportional, không còn wasted-space feel như trước (ZWS value hoặc chỉ `N pending`).
