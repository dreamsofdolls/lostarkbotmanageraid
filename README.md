...
- **Roster section header**: non-inline field với name = `📁 <accountName> (<displayName>)` và value = `**N pending** · 🔄 <relative>`. Cả 2 dòng đều có info (không dùng ZWS value) nên Discord's 2-line height không còn bị wasted. 1 user có 2 roster (main + alt) hiện 2 section riêng biệt với accountName phân biệt. Rosters cùng 1 user group consecutive, sort by user-total pending desc rồi per-roster pending desc rồi alphabetical tie-break.
