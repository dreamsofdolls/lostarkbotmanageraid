...
Mỗi khi đổi command schema (thêm/sửa option, đổi name, thêm autocomplete), chạy `npm run deploy:commands` **một lần** từ máy dev với `.env` chứa production `DISCORD_TOKEN` + `CLIENT_ID` + `GUILD_ID`. Không cần redeploy Railway chỉ để register schema.
