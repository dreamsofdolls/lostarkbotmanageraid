...
4. Railway tự build bằng `Dockerfile` (node:20-slim, `npm install --omit=dev`). Start command (từ `railway.toml`) là `node src/deploy-commands.js; node src/bot.js` — **auto-register slash command schema mỗi lần boot**, sau đó start bot **độc lập**. Nếu schema register fail, bot vẫn chạy với schema cũ (fail-soft, không block).
5. Restart policy: `ON_FAILURE`, tối đa 3 lần retry (xem `railway.toml`).

Không cần chạy `deploy:commands` thủ công cho Railway deploy — mỗi lần redeploy schema tự được sync với code mới. Nếu muốn skip register (edge case), đổi `startCommand` về chỉ `node src/bot.js`.
