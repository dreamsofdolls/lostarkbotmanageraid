...
- **Per-roster section header**: **inline field + 2 spacer fields** fill 1 row (3-inline-per-row Discord layout). Name chứa toàn bộ info `📁 <accountName> (<displayName>) · 🔄<relative>` (sync badge nằm ngang với tên roster). Inline header compact hơn non-inline field (non-inline có 2-line reserve + generous padding, inline chỉ 1 line + tight inter-row padding) - approximate `/raid-status`'s tight density. Pending count đã bỏ khỏi section header vì footer aggregate counts covers already. Non-opted-in user không có sync badge → name chỉ có `📁 <accountName> (<displayName>)`. Trade-off: inline header chỉ chiếm 1/3 row width (~130px), label dài có thể wrap 2 dòng trong narrow column - nhưng vẫn tight hơn non-inline + ZWS approach.
