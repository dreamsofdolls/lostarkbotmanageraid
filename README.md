...
- **Roster section header**: inline field `📁 <accountName> (<displayName>) · N pending · 🔄 <relative>` ở column 1 + 2 spacer fields fill row. Dùng inline (không phải non-inline) để Discord render compact - non-inline + ZWS value sẽ tạo gap 1-line wasted giữa header và char cards. 1 user có 2 roster (main + alt) hiện 2 section riêng biệt với accountName phân biệt. Rosters cùng 1 user group consecutive, sort by user-total pending desc rồi per-roster pending desc rồi alphabetical tie-break.
- **Sync badge trong section header**: opted-in user có sync data hiện `🔄 5m` / `🔄 2h` / `🔄 3d` (relative time tự compute, English units để giữ segment ngắn). Opted-in chưa sync lần nào → `🔄 never`. Non-opted-in → không hiện segment.
- **Field budget accounting**: mỗi roster cost = 3 header-row fields (header + 2 spacers) + N char fields + ceil(N/2) spacer fields. 6-char roster = 12 fields, 4-char = 9, 1-char = 6. Pagination check trước khi start roster (`fieldCount + need > 25`) để section header không bị orphan cuối embed trước. Typical guild 24 pending × 6 rosters ~ 48 fields → 2 embed (initial + 1 follow-up ephemeral).
- **Footer**: `Raid Manager scan · N user cần nhắc · M roster` - breakdown user/roster action-able hơn "N char(s) cần update" (Raid Manager cần biết sẽ ping bao nhiêu người).
