...
- **Auto-manage piggyback (Phase 2)**: nếu `User.autoManageEnabled = true`, gather phase pull thêm bible logs cho toàn roster (gate bởi cooldown **per-user** + in-flight guard của `/raid-auto-manage` - Raid Manager `30s`, user thường `15 phút`, share cùng `acquireAutoManageSyncSlot`). Apply phase merge cả refresh data lẫn auto-manage report vào cùng fresh doc, lưu trong 1 save → atomic. Nếu cooldown chưa hết hoặc gather throw → silent skip auto-manage, vẫn render với cached data. User opt-in chỉ cần gõ `/raid-status` để vừa xem progress vừa auto-pull bible - không cần `action:sync` riêng.
- **Status hot-path budget**: `/raid-status` chỉ chờ auto-manage piggyback tối đa `STATUS_AUTO_MANAGE_PIGGYBACK_BUDGET_MS = 2500ms`. Nếu bible log sync lâu hơn budget, command render cached data ngay, stamp cooldown để tránh retry spam, rồi tiếp tục apply sync nền khi gather xong. Roster metadata refresh vẫn foreground vì iLvl/class ảnh hưởng trực tiếp trang status.
- Fetch failure (Bible down, rate limit) → log warning + skip account đó, command vẫn render cached data.
- **Freshness + countdown line mỗi account embed**: dưới summary line có thêm `📥 Last updated Nh ago · ⏳ Next refresh in Xh · 🔄 Last synced Nm ago · ⏳ Next sync in Ym`. Khi cooldown đã hết show `✅ Refresh ready` / `✅ Sync ready` - user biết ngay chạy lại `/raid-status` sẽ thực sự fetch data mới (không phải hit cache) thay vì đoán mò tại sao iLvl hoặc raid progress chưa update. `📥` = roster metadata gate 2h (always visible), nhưng nếu lần refresh gần nhất fail thì countdown tôn trọng failure cooldown 5 phút qua `lastRefreshAttemptAt`. `🔄` = auto-manage bible log sync gate **per-user** (`30s` cho Raid Manager, `15 phút` cho user thường; chỉ hiện khi user opt-in).
...
- **Mongo prefilter theo raid floor + stale accounts**: `/raid-check` query kéo user có ít nhất 1 character đạt iLvl thấp nhất của raid đang scan (ví dụ Kazeros Hard vẫn lấy floor Kazeros Normal 1710 để còn render low/not-eligible context) **hoặc** có account stale/unrefreshed cần lazy-refresh trước khi scan, miễn là failure cooldown 5 phút đã hết. Kết quả render không đổi vì JS vốn bỏ char dưới raid floor, nhưng command tránh đọc/pre-refresh các roster fresh chắc chắn không liên quan mà vẫn không giấu char vừa honed lên nhưng DB còn iLvl cũ.
...
- **Pagination buttons + session**: `◀ Previous` / `Next ▶` (từ shared helper `buildPaginationRow` - `/raid-status` cũng dùng cùng helper, chỉ khác customId prefix) cycle giữa roster pages. Session timeout **5 phút** (`RAID_CHECK_PAGINATION_SESSION_MS`), hết hạn thì disable buttons + footer đổi thành `⏱️ Session đã hết hạn (300s) · Dùng /raid-check để xem lại`. Collector lock theo người chạy command - user khác click prev/next sẽ nhận ephemeral reject.
- **Freshness badges trong roster header (explicit labels + countdown)**: value line dùng text rõ ràng thay vì emoji+số cryptic. `📥 Last updated <relative> ago` = roster data refresh (iLvl/class, applies ALL users) từ `/raid-status` lazy refresh hoặc pre-scan refresh của `/raid-check`. `🔄 Last synced <relative> ago` = auto-manage bible log sync (chỉ opted-in). Opted-in chưa sync → `🔄 Never synced`. Non-opted-in → không hiện segment `🔄`. Mỗi badge kèm countdown `⏳ Next refresh in Xh` / `⏳ Next sync in Ym/s` khi cooldown còn active (roster refresh 2h hoặc 5m failure cooldown; auto-manage sync **per-user**: Raid Manager 30s, non-manager 15m) hoặc `✅ Refresh ready` / `✅ Sync ready` khi hết cooldown - user không phải đoán mò tại sao Sync button bấm xong data không đổi. Ví dụ: `4 ⚪ · 1 🟡 · 1 🟢 · 📥 Last updated 30m ago · ⏳ Next refresh in 1h30m · 🔄 Last synced 3m ago · ⏳ Next sync in 12m`. Dropped `avg iLvl` per Traine - text phrase rõ nghĩa cho Raid Manager, scan-friendly khi quét nhiều roster consecutive.
- **👑 Manager crown ở roster header (không prefix từng char)**: roster thuộc Raid Manager (Discord ID nằm trong env `RAID_MANAGER_ID`) swap icon header từ 📁 sang 👑 - 1 crown per roster, không stamp từng char. Lý do: 6 char cùng prefix crown khiến list trở nên rối, và char line tương lai sẽ dùng class-icon thay text nên không còn room cho prefix. Header-level crown giữ được signal "manager roster" mà không cost scan clarity. Non-manager roster giữ 📁. Logic gom chung với `isManagerId()` helper (`src/services/manager.js`) nên cooldown 30s + icon 👑 đều cùng 1 allowlist - add/remove manager chỉ cần update env rồi redeploy. Single-tier (không phân Owner vs Manager) để keep UX phẳng với 2-person deployment.
...
- **✏️ Edit button** (Apr 2026): Raid Manager bấm → ephemeral follow-up cascading select flow (**user** → **char** → **raid** → **status buttons** → **gate buttons** khi Process). Full `/raid-set` parity (Complete / Process+gate / Reset), reuse `applyRaidSetForDiscordId` helper không có write logic mới. **Auth rule per-char**: nếu owner đã bật `autoManageEnabled=true` AND `character.publicLogDisabled=false` → skip khỏi dropdown (bible auto-sync owns char đó, manual edit sẽ bị ghi đè lần tick sau). Char `publicLogDisabled=true` (bible báo 'Logs not enabled' lần sync gần nhất) → vẫn editable vì bible không bao giờ reach được. Non-auto-sync user → tất cả char editable. `publicLogDisabled` stamp/clear tự động bởi `applyAutoManageCollected` (flip true khi gặp `isPublicLogDisabledError`, flip false khi log fetch success). Char-select label hiện suffix `· log off (manager only)` cho auto-sync char exception; user-select label hiện `· auto-sync` cho auto-sync user. Icon 🔒 vs ⚔️ trên char option để nhìn nhanh. Session: collector timeout 5 phút, interaction token Discord 15 phút. Khi token expired hoặc `component.message.edit()` fail → bot post public channel message tag leader (`<@leaderId> Edit session hết hạn, gõ lại /raid-check...`, auto-delete 30s) để click cuối cùng không silent. Raid select filter theo `char.itemLevel >= entry.minItemLevel` để tránh "mark Nightmare cho char 1710" footgun. Auto-refresh parent embed intentional skip V1 → confirmation hiện hint rerun `/raid-check`.
...
- `/raid-channel config action:schedule-on` - bật auto-cleanup mỗi 30 phút (slot :00 và :30 giờ VN, kèm biển báo tone theo lượng rác 5 phút tự xóa)
- `/raid-channel config action:schedule-off` - tắt auto-cleanup 30 phút
...
- Difficulty: `normal` / `nor` / `nm` · `hard` / `hm` · `nightmare` / `9m` (lưu ý: `nm` alias cho Normal, Nightmare chỉ còn `9m`)
...
**30-min auto-cleanup + 4-bucket tone variants (Apr 2026)**: auto-cleanup cadence evolution - daily 00:00 VN → hourly → **mỗi 30 phút** (slot :00 và :30 giờ VN). `lastAutoCleanupKey` format evolution: `YYYY-MM-DD` → `YYYY-MM-DDTHH` → `YYYY-MM-DDTHH:MM` (MM ∈ {00, 30}). Tick cadence 30 phút align 1-1 với slot boundary. Flow: chạy `cleanupRaidChannelMessages(channel)` trước, **luôn post 1 biển báo sau đó** (giọng Dusk signed Artist, no stage-direction italic) + `setTimeout(5 min)` fire-and-forget xóa biển. **4-bucket variant pool** (`CLEANUP_NOTICE_VARIANTS_BY_BUCKET` + `pickCleanupNoticeContent(deleted)`): `empty` (deleted=0, hài lòng channel sạch sẵn) / `trivial` (1-5, nhẹ nhàng) / `normal` (6-20, càu nhàu chuẩn) / `heavy` (21+, exasperated). Mỗi bucket 3 variants, random pick per-fire → channel tone lived-in không bị đơn điệu. Lý do vẫn post khi idle: silence trong scheduled window đọc giống 'bot offline/broken'; notice idle double làm heartbeat + idle-state marker. Trade-off: ~50ms gap giữa cleanup complete và notice post - user post vào đúng khe đó thì tin họ sống sót (không bị nhầm là rác vừa dọn). Legacy hour-keys cũ trong Mongo sẽ không match slot-key mới → 1 lần re-sweep sau deploy, harmless. Manual `/raid-channel config action:cleanup` KHÔNG post biển báo (user đã biết mình chạy).

**Artist quiet hours 03:00-08:00 VN + ceremonial bedtime/wake-up (Apr 2026)**: cùng scheduler tick với auto-cleanup nhưng dispatch 3 nhánh trong `runAutoCleanupTick`. (1) **Quiet branch** `VN hour ∈ [3, 8)` → skip sweep + skip hourly notice. Tick đầu tiên trong window post 1 **bedtime embed** (`BEDTIME_NOTICE_VARIANTS` - 3 variants pool, TTL 5 phút, dedup `GuildConfig.lastArtistBedtimeKey = 'YYYY-MM-DD'` VN calendar). Các tick sau silent no-op. Bedtime disable qua `/raid-announce artist-bedtime action:off` → stamp day key im lặng, quiet-hours behavior (no sweep) vẫn áp dụng. (2) **Wake-up branch** tick đầu `VN hour ≥ 8` trong ngày chưa wake-up → sweep toàn bộ non-pinned 1 lần catch-up đêm qua + post **wake-up embed combined** (`WAKEUP_NOTICE_VARIANTS_BY_BUCKET` - 4-bucket giống cleanup nhưng tone 'vừa ngủ dậy', TTL 10 phút, dedup `lastArtistWakeupKey`); cũng stamp `lastAutoCleanupKey` slot hiện tại nên tick 8:30 không double-sweep. Wake-up disable → sweep vẫn chạy, chỉ skip embed greeting. (3) **Normal branch** `VN hour ≥ 8 AND wake-up đã fire today` → hourly-cleanup path hiện có. **Message parsing (raid clear post) vẫn active 24/7** - parse + DM + update DB unaffected; chỉ scheduler-side ngủ. Tick ở slot `:00`/`:30` trong [3:00, 8:00) bắt gặp `lastArtistBedtimeKey === today` → silent return (không cleanup, không notice). Registry entries `artist-bedtime` + `artist-wakeup` hiển thị countdown trong `/raid-announce show` tới next 03:00/08:00 VN boundary (= 20:00/01:00 UTC). Admin `action:schedule-off` tắt toàn bộ 3 branch; per-guild disable bedtime/wake-up riêng lẻ qua `/raid-announce {type} action:off`.

**Welcome repin sau deploy**: welcome embed đã cập nhật (thêm section liệt kê 4 announcement types + cadence 30-min). Các server đã pin welcome cũ thì **admin cần chạy `/raid-channel config action:repin` một lần** để refresh nội dung. Repin là safe-order (post + pin fresh trước, unpin stale sau) nên không mất welcome giữa chừng.
...
2. **Auto-cleanup notice** (TTL 5 phút): mỗi 30 phút (slot :00 và :30 giờ VN) sau khi cleanup chạy. 4-bucket tone với 3 variants random pick mỗi bucket: `empty` (channel sạch sẵn) / `trivial` 1-5 tin / `normal` 6-20 tin / `heavy` 21+ tin.
...
- `set-channel` - set `announcements.<type>.channelId` (chỉ overridable types, cần option `channel`; bot phải có `View Channel` + `Send Messages` ở channel đích)
...
- **Phase 1 - manual**: `/raid-auto-manage action:sync` bất cứ lúc nào (cooldown per-user: Raid Manager `30s`, user thường `15 phút`)
...
- **Roster fallback cache trong một lượt gather**: nếu direct bible meta lookup fail cho nhiều char cùng account (ví dụ tên lưu thiếu dấu), fallback roster seed chỉ fetch một lần rồi reuse cho các char còn lại trong lượt sync đó. Log API vẫn per-char như cũ vì clear history là dữ liệu riêng của từng char.
- **Per-user throttle (30s/15 phút + in-flight guard)**: dựa trên `User.lastAutoManageAttemptAt` + in-memory `Set` theo `discordId`. `action:sync` spam → ephemeral reject với remaining time (`30s` cho Raid Manager, `15 phút` cho user thường). In-flight song song cùng user → reject ngay. `action:on` có logic mềm hơn: in-flight thì reject hard, nhưng cooldown thì vẫn flip flag (UX: "bật" không bao giờ fail), chỉ skip cả probe lẫn sync và báo user chờ đúng khoảng còn lại. Cần guard vì mỗi sync chạy scrape+paginate cho toàn roster (N-char × HTTP calls) - `bibleLimiter` chỉ cap concurrency, không chặn total queue. **Probe cancel/timeout/error cũng stamp `lastAutoManageAttemptAt`** - không thì user spam `action:on` + bấm Huỷ liên tục sẽ bypass được cooldown (probe HTTP vẫn tốn quota).
...
| `RAID_MANAGER_ID` | ❌ (recommended) | empty | Discord user IDs cho phép gọi `/raid-check`, comma-separated. Empty/missing → `/raid-check` reject mọi invocation + warn ở boot. **Also drives manager privileges**: 30s auto-manage sync cooldown (vs 15m default) + `👑` header icon (thay 📁) cho roster thuộc manager trong `/raid-check` + `/raid-status` embeds |
