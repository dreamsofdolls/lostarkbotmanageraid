...
- Raid signup board preview (`/raid-schedule-preview`): creates Support/DPS slots, waitlist, RSVP, auto-lock, and derives party size from the selected raid (Act 4/Kazeros = 8, Serca = 4)
...
| `/raid-schedule-preview` | Raid Manager | Preview command for a public raid signup board: `raid`, `mode`, `when`, optional `auto_lock`/`title`. Party size is derived from the raid catalog: Act 4/Kazeros use 8 slots, Serca uses 4. Members join from saved rosters; the board manages Support/DPS slots, waitlist, RSVP, room visibility, and a lead-only Manage menu for Lock/Unlock, End, Set room, Edit time, and Cancel. End writes clears through `/raid-set`. |
