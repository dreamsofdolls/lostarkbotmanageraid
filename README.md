...
- Auto-sync from lostark.bible logs (opt-in via `/raid-auto-manage action:on`) with silent background sync every 24 hours (due users are checked every 5 minutes in small batches; transient errors use bounded backoff), plus a scoped Solo Local Reader launched from `/raid-status`; the alternative full local-sync mode (`action:local-on`) reads the active raid week from `encounters.db`. The browser remembers the file handle, watches SQLite revisions in near real time (sub-500 ms visible fallback), discards superseded scans, and verifies the current file header immediately before sending a small delta preview; changed files still settle and rebuild before handoff. The API acknowledges as soon as that preview is durably stored while Discord DM delivery continues in the background. Raid progress is written only after the owner confirms in `/raid-status` → `🗃️ Local Sync`. A confirmed Full Local Sync also propagates that encounter's cumulative Gate progress to party characters registered under Local Sync or Bible Auto Sync owners, but only while the target raid has no current-week progress at all; Solo Local Reader never propagates party progress. Party membership comes from the client-readable `players` snapshot and is not cryptographically attested by Artist; the server limits each evidenced source Gate to 15 propagated targets, requires that source Gate to apply first, and re-checks target consent and untouched progress before every write.
