...
| `/raid-schedule-preview` | `create`/manage: Raid Manager · `show`: anyone | Preview command for a public raid signup board. `create`: `raid`, `mode`, `when`, optional `auto_lock`/`title` (party size derived from the catalog: Act 4/Kazeros = 8, Serca = 4). Members join from saved rosters; the board manages Support/DPS slots, waitlist, RSVP, room visibility, and a lead-only Manage menu for Lock/Unlock, End, Set room, Edit time, Cancel, 🧩 Turns, ➕ Add member, and 👋 Kick. **Turns** = bus model: the lead arranges signups into multiple turns (the same player can run several) by ticking the signup pool. **Add member** lets the lead slot someone in directly (pick a user, then an iLvl-eligible character from their roster; pings them; works while locked). **Kick** drops one or more people (multi-select); dropping a slot-holder auto-promotes the next waitlister. `show` posts a public turn plan (who plays which character + class + sup/dps). End writes clears through `/raid-set`. |
