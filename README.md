...
- **Per-roster section header**: non-inline field với name chứa toàn bộ info `📁 <accountName> (<displayName>) · 🔄<relative>` (sync badge nằm ngang với tên roster). Value = ZWS. Pending count đã bỏ khỏi section header vì footer aggregate counts covers already. Non-opted-in user không có sync badge → name chỉ có `📁 <accountName> (<displayName>)`.
