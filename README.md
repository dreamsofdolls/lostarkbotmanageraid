...
- Auction bid calculator (`/raid-auction`): given an item's market value and party size (4/8), computes the recommended bid (formula ported from la-utils) plus each member's cut and the winner's estimated profit
...
| `/raid-auction` | anyone (self) | Auction bid calculator for shared raid loot. `players:4/8` + `market_value:<gold>` (+ optional `profit`, default on). Formula ported from la-utils: `floor(0.95 × value × (N-1)/N)`, ×0.92 in profit mode. Replies publicly (so the whole party sees the bid) with the bid, each member's cut, and the winner's estimated profit; input errors stay ephemeral. |
