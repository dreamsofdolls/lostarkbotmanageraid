...
- **Mongo prefilter theo raid floor**: `/raid-check` query chỉ kéo user có ít nhất 1 character đạt iLvl thấp nhất của raid đang scan (ví dụ Kazeros Hard vẫn lấy floor Kazeros Normal 1710 để còn render low/not-eligible context). Kết quả render không đổi vì JS vốn bỏ char dưới raid floor, nhưng command tránh đọc/pre-refresh các roster chắc chắn không liên quan.
