...
| `/raid-status` | anyone (self) | View raid progress, paginated 1 roster/page; the view dropdown holds four screens (📋 Raid · 📝 Side tasks · 💰 Gold & difficulty · 🗃️ Local Sync, the last one being `/raid-sync` rendered in this same frame · always listed since Discord cannot disable a single select option, and it renders a no-button "turn it on" card when no sync mode is active); roster iLvl/class auto-refreshes in the background and can be refreshed immediately from the Refresh roster button; Bible Auto-sync users also get a Local Reader launcher whose private link detects explicit Solo clears and sends a preview to Discord for confirmation; per-char + per-account weekly gold rollup with bound gold shown via the `🔒` tail, with per-char gold hidden when a specific raid filter is active; Gold view includes a per-character difficulty dropdown that applies immediately before a raid runs or queues for the next weekly reset after it ran; a `🗓️ Raid của tôi` dropdown lists the `/raid-schedule-preview` events you are in (self-join or manager-added) and opens a personal detail (time, room if in comp, your turns + teammates) |
| `/raid-sync` | anyone (self) | Review the latest stored preview in a private durable console, then Sync / Cancel / Refresh it. Preview jobs last 2 hours; a newer scan supersedes an older pending one, interrupted applies recover automatically, and transient DB writes can be retried without losing successful writes. With no actionable preview, and immediately after Sync or Cancel, the same message becomes the full `/raid-status` UI opened on its `🗃️ Local Sync` view, one dropdown hop from Raid/Task/Gold with their normal pagination, filters, roster refresh, and Local Reader controls. A preview still awaiting review deliberately stays in the standalone console: its buttons route through the global interaction router and keep working after the 10-minute `/raid-status` collector would have expired. Refreshing or running the command again reads the latest DB state. |
