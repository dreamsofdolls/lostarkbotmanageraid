...
- **Per-char display**: mỗi char line format `<gate icons> name iLvl` (flat text - không bold, không backtick). Icons 1-per-gate: 🟢 = gate đó done ở mode đang scan, ⚪ = gate đó chưa done (hoặc done ở mode khác - mode-switch wipe sẽ xảy ra khi `/raid-set` ở mode này). Ví dụ Kazeros 2-gate: `🟢⚪ Clauseduk 1744` = G1 done G2 chưa. iLvl round integer, bỏ decimal noise (`1744` thay `1744.17`).
- **2-column layout**: chars pair lên thành `char1 · char2` mỗi row để nửa chiều dọc embed (12 chars → 6 rows thay 12 rows). Odd char còn lại solo cuối. Joined-value approach (không inline-field) để 1 roster 20+ chars vẫn fit 1 field, không overflow 25-field limit.
- **Per-roster grouping**: field name format `📁 <accountName> (<displayName>) · N pending · 🔄 <relative>`. Roster name là primary anchor (Raid Manager nhớ tên account hơn Discord name khi scan), displayName đi trong ngoặc đơn. User có 2 roster (main + alt) hiện 2 field riêng biệt với accountName phân biệt. Rosters cùng 1 user group consecutive, sort by user-total pending desc rồi per-roster pending desc rồi alphabetical tie-break.
- **Sync badge trong field name**: opted-in user có sync data hiện `🔄 5m` / `🔄 2h` / `🔄 3d` (relative time tự compute, English units để giữ segment ngắn). Opted-in chưa sync lần nào → `🔄 never`. Non-opted-in → không hiện segment. ⚡ partial count đã bỏ khỏi field name vì 🟡 gate icons trong value đã surface partial state per-char - aggregate ở header là redundant.
