...
| `/raid-check` | Raid Manager | Scan rosters for pending chars; **Sync-check all** pulls Bible logs across all raids and all saved rosters whose owners enabled Auto-sync (Local Sync users are skipped); it sits on the button row in every view, filtered or not. Reopen `/raid-check` after syncing to load the updated progress. Refresh roster pulls iLvl/class for the current roster; a 📋 teams dropdown lists every active signup board in the guild (any Manager's) and opens its comp + turn plan ephemerally (spills across extra dropdowns past 25 events). |
