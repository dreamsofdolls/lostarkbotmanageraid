...
- **Footer legend**: `🟢 done · 🟡 partial · ⚪ pending` - reuses `/raid-status`'s `STATUS_FOOTER_LEGEND` const (single source of truth cho English legend của cả 2 paginated command). User/roster scan counts đã bỏ cho gọn.
