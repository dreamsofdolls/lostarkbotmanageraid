...
| `/raid-check` | Raid Manager | Scan rosters for pending chars; **Sync-check all** on the overview pulls Bible logs across all raids and all saved rosters whose owners enabled Auto-sync (Local Sync users are skipped). Clear the user filter to return to this button; reopen `/raid-check` after syncing to load the updated progress. Refresh roster pulls iLvl/class for the current roster, and Edit opens cascading selectors; a 📋 teams dropdown lists every active signup board in the guild (any Manager's) and opens its comp + turn plan ephemerally (spills across extra dropdowns past 25 events). |
