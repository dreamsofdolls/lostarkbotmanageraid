...
- **Per-char card (inline field)**: mỗi character = 1 Discord inline field. Field name chứa TẤT CẢ info `<charName> · <iLvl> · <icon> <done>/<total>` (ví dụ `Qiylyn · 1743 · ⚪ 0/2`) - icon + counter nằm ngang cùng dòng với name, không xuống value line bên dưới. Field value = ZWS (Discord yêu cầu non-empty). **Aggregate 3-state icon** qua helper `pickProgressIcon`: 🟢 (all gates done), 🟡 (partial - ít nhất 1 gate done), ⚪ (none done). iLvl round integer (`1744` thay `1744.17`).
...
- **Footer legend**: `🟢 cleared · 🟡 đi mới G1 · ⚪ chưa đi · N users cần nhắc · M rosters` - mirror `/raid-status`'s footer-legend pattern. Prefix giải thích 3 aggregate colors (🟢 = hoàn thành tất cả gate, 🟡 = mới xong 1 gate, ⚪ = chưa chạm gate nào) để Raid Manager khỏi đoán. Suffix cung cấp scan breakdown action-able (biết ping bao nhiêu người). Label `đi mới G1` specific cho 2-gate raids hiện tại; khi có 3-gate raid sẽ generalize thành "đang đi" hoặc tương tự.
