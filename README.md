...
- Raid signup board preview (`/raid-schedule-preview`): Support/DPS slots, waitlist, RSVP, auto-lock, a lead Manage menu, multi-turn (bus) team assignment, and a `show` turn plan; party size is derived from the selected raid (Act 4/Kazeros = 8, Serca = 4)
...
| `/raid-schedule-preview` | `create`/manage: Raid Manager · `show`: anyone | Preview command for a public raid signup board. `create`: `raid`, `mode`, `when`, optional `auto_lock`/`title` (party size derived from the catalog: Act 4/Kazeros = 8, Serca = 4). Members join from saved rosters; the board manages Support/DPS slots, waitlist, RSVP, room visibility, and a lead-only Manage menu for Lock/Unlock, End, Set room, Edit time, Cancel, and 🧩 Turns. **Turns** = bus model: the lead arranges signups into multiple turns (the same player can run several) by ticking the signup pool. `show` posts a public turn plan (who plays which character + class + sup/dps). End writes clears through `/raid-set`. |
