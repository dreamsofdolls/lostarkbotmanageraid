...
- **User filter dropdown**: first option `🌐 All users (N pending)` reset filter. Tiếp theo top-24 users sort theo pending desc (`👤 displayName (N pending)`). Discord cap 25 options. Selection narrow pages về chỉ user đó's rosters, reset currentPage=0. `default: true` trên selected option giúp preserve state qua Prev/Next clicks.
- **Chunked pages (2 rosters each)**: mỗi embed page stack 2 roster sections. Section = non-inline header (name = `📁 accountName (displayName)`, value = `N pending · 🔄<relative>`) + inline char cards 2-col. User có 4 rosters → 2 pages (2+2). User có 3 rosters → 2 pages (2+1). Chunking giữ rosters cùng user consecutive.
