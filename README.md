...
- **Header summary (1 dòng)**: `<pending>/<eligible> pending (<pct>%) · iLvl ≥ <X> · 🟢 <done> · 🟡 <started> · ⚪ <untouched>`. Ratio pending/eligible là con số chính (action-relevant), 🟢/🟡/⚪ distribution bổ sung info "bao nhiêu đã start vs chưa chạm" mà ratio không surface được.
- **Per-char card (inline field)**: mỗi character = 1 Discord inline field, mirror pattern của `/raid-status` để hai command thuộc cùng visual system. Field name `<charName> · <iLvl>` được Discord auto-bold → char name là scan anchor rõ ràng. Field value `<gate icons> <raid label> · <done>/<total>` format giống `/raid-status` (ví dụ `🟢⚪ Act 4 Normal · 1/2`). Per-gate icons giữ nguyên để partial-done state (`🟢⚪` = G1 done G2 chưa) vẫn glance-able. iLvl round integer (`1744` thay `1744.17`).
- **2-column layout via Discord inline fields**: Discord default pack 3 inline fields/row; chèn zero-width-space spacer field (`{name:'​', value:'​', inline:true}`) giữa mỗi cặp char để force 2-per-row với gap thở - kỹ thuật y hệt `/raid-status`. Odd char cuối cùng cặp với 1 spacer để không bị Discord stretch full-width.
- **Multi-embed layout (1 roster = 1 embed)**: reply là message chứa nhiều embed - `embeds[0]` là summary (chỉ title + description stats, không có field), `embeds[1..N]` mỗi embed là 1 roster với title = `📁 <accountName> (<displayName>) · N pending · 🔄 <relative>` và fields = char cards inline. Lý do không dùng field làm section header: Discord reserve nguyên paragraph height (~24px) cho empty field value dù có ZWS, tạo gap rõ ràng giữa header và char cards. Embed title → fields spacing chỉ ~8px, tight hơn nhiều. 1 user có 2 roster (main + alt) = 2 roster embeds riêng biệt. Rosters cùng 1 user group consecutive, sort by user-total pending desc rồi per-roster pending desc rồi alphabetical tie-break.
- **Sync badge trong embed title**: opted-in user có sync data hiện `🔄 5m` / `🔄 2h` / `🔄 3d` (relative time tự compute, English units để giữ segment ngắn). Opted-in chưa sync lần nào → `🔄 never`. Non-opted-in → không hiện segment.
- **Message batching**: Discord cap 10 embed/message. First message = `editReply` với 10 embed đầu + action row buttons. Tiếp theo là ephemeral `followUp` batches of 10 (không có button để tránh duplicate row). Typical guild 6 rosters → 7 embed (1 summary + 6 roster) → fit 1 message. > 9 rosters mới split. Per-roster field budget: 1 char = 3 field (card + spacer + card-or-spacer) nên 6-char roster = 9 field, under 25-per-embed limit dễ dàng.
- **Footer ở embed cuối cùng**: `Raid Manager scan · N user cần nhắc · M roster` - chỉ attach vào embed cuối để sit ở visual bottom bất kể multi-message split. Action-able hơn "N char(s) cần update" vì Raid Manager quan tâm sẽ ping bao nhiêu người, không phải bao nhiêu char.
- Multi-embed pagination khi > 25 fields hoặc > 5500 chars - follow-up messages ephemeral. Action row (2 buttons bên dưới) attach vào embed đầu tiên; continuation embeds không có button.
- **🔔 Remind button**: bấm → bot DM mỗi pending user list chars của họ ở format `<gate icons> **name** iLvl` + 3 update paths (post /raid-channel, /raid-set manual, /raid-auto-manage opt-in). Rate-limit DM qua `discordUserLimiter` (max 5 concurrent). User tắt DM → failed list hiện trong ephemeral reply.
