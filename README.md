...
4. Railway tự build bằng `Dockerfile` (node:20-slim, `npm install --omit=dev`). Start command (từ `railway.toml`) là `node src/deploy-commands.js && node src/bot.js` — **auto-register slash command schema mỗi lần boot**, sau đó mới start bot.
