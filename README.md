...
| `/raid-auction` | anyone (self) | Auction bid calculator for shared raid loot. `players:4/8` + `market_value:<gold>` (+ optional `profit`, default on). Formula ported from la-utils: `floor(0.95 × value × (N-1)/N)`, ×0.92 in profit mode. Replies ephemerally with the bid, each member's cut, and the winner's estimated profit. |
