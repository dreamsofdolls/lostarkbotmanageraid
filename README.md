...
- **Auto-manage piggyback (Phase 2)**: nếu `User.autoManageEnabled = true`, gather phase pull thêm bible logs cho toàn roster (gate bởi cooldown 15 phút + in-flight guard của `/raid-auto-manage` - share cùng `acquireAutoManageSyncSlot`). Apply phase merge cả refresh data lẫn auto-manage report vào cùng fresh doc, lưu trong 1 save → atomic. Nếu cooldown chưa hết hoặc gather throw → silent skip auto-manage, vẫn render với cached data. User opt-in chỉ cần gõ `/raid-status` để vừa xem progress vừa auto-pull bible - không cần `action:sync` riêng.
...
- **👑 Manager character prefix**: char thuộc roster của Raid Manager (Discord ID nằm trong env `RAID_MANAGER_ID`) render với prefix `👑 <charName>` ở cả `/raid-check` và `/raid-status`. Non-manager char render bình thường. Logic gom chung với `isManagerId()` helper (`src/services/manager.js`) nên cooldown 30s + icon 👑 đều cùng 1 allowlist - add/remove manager chỉ cần update env rồi redeploy. Single-tier (không phân Owner vs Manager) để keep UX phẳng với 2-person deployment.
...
- **Phase 1 - manual**: `/raid-auto-manage action:sync` bất cứ lúc nào (15-min cooldown per user)
...
| `RAID_MANAGER_ID` | ❌ (recommended) | empty | Discord user IDs cho phép gọi `/raid-check`, comma-separated. Empty/missing → `/raid-check` reject mọi invocation + warn ở boot. **Also drives manager privileges**: 30s auto-manage sync cooldown (vs 15m default) + `👑` prefix trên character name trong `/raid-check` + `/raid-status` embeds |
