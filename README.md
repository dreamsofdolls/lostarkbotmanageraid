...
- Auto-sync from lostark.bible logs (opt-in via `/raid-auto-manage action:on`) with silent background sync every 24 hours (due users are checked every 5 minutes in small batches; transient errors use bounded backoff), plus a scoped Solo Local Reader launched from `/raid-status`; the alternative full local-sync mode (`action:local-on`) reads the active raid week from `encounters.db`. The browser remembers the file handle, watches SQLite revisions in near real time (sub-500 ms visible fallback), discards superseded scans, and verifies the current file header immediately before sending a small delta preview; changed files still settle and rebuild before handoff. The API acknowledges as soon as that preview is durably stored while Discord DM delivery continues in the background. Raid progress is written only after the owner confirms in `/raid-status` → `🗃️ Local Sync`. A confirmed Full Local Sync also propagates that encounter's cumulative Gate progress to party characters registered in any Artist roster, whatever sync mode their owner uses, but only while the target raid has no current-week progress at all; the Local Reader preview names those registered party members and the synced card lists the ones it wrote. Solo Local Reader never propagates party progress. Party membership comes from the client-readable `players` snapshot and is not cryptographically attested by Artist; the server limits each evidenced source Gate to 15 propagated targets, requires that source Gate to apply first, and re-checks untouched progress before every write.
...
- Raid Manager tier: shorter sync cooldown, `👑` roster header icon
...
| `/raid-status` | anyone (self) | View raid progress, paginated 1 roster/page; the view dropdown holds four screens (📋 Raid · 📝 Side tasks · 💰 Gold & difficulty · 🗃️ Local Sync · always listed since Discord cannot disable a single select option, and it renders a no-button "turn it on" card when no sync mode is active). Local Sync reviews the latest stored preview, then Syncs / Cancels / Refreshes it in this same frame; preview jobs last 2 hours, newer scans supersede older pending previews, interrupted applies recover automatically, and transient DB writes can be retried without losing successful writes. Roster iLvl/class auto-refreshes in the background and can be refreshed immediately from the Refresh roster button; Bible Auto-sync users also get a Local Reader launcher whose private link detects explicit Solo clears and sends a preview to Discord for confirmation; per-char + per-account weekly gold rollup shows bound gold through the `🔒` tail, with per-char gold hidden when a specific raid filter is active; Gold view includes a per-character difficulty dropdown that applies immediately before a raid runs or queues for the next weekly reset after it ran; a `🗓️ Raid của tôi` dropdown lists the `/raid-schedule-preview` events you are in (self-join or manager-added) and opens a personal detail (time, room if in comp, your turns + teammates) |
...
| `/raid-check` | Raid Manager | Scan rosters for pending chars from iLvl 1720 up (lower characters stay out of its cards, counts and roster pages); **Sync-check all** pulls Bible logs across all raids and all saved rosters whose owners enabled Auto-sync (Local Sync users are skipped); it sits on the button row in every view, filtered or not. Reopen `/raid-check` after syncing to load the updated progress. Refresh roster pulls iLvl/class for the current roster; a 📋 teams dropdown lists every active signup board in the guild (any Manager's) and opens its comp + turn plan ephemerally (spills across extra dropdowns past 25 events). |
| `/raid-auto-manage` | anyone (self) | `on` / `off` / `sync` / `status` for bible-log auto-reconcile · `local-on` / `local-off` for full `encounters.db` Local Reader mode (mutex with bible; confirmation lives in `/raid-status` → `🗃️ Local Sync`) · `reset` to wipe your own raid progress + sync state (2-step confirm) |
...
4. **Shared write paths.** `applyRaidSetForDiscordId` is reused by `/raid-set`, the text monitor, and Local Sync apply logic - new UIs never re-implement raid mutation.
...
- Bible auto-sync can't reach a character with Public Log OFF. Its owner records that progress through `/raid-set`, the text monitor or Local Sync; the Refresh roster buttons only update roster metadata such as iLvl/class.
...
- Remembering and continuously watching `encounters.db` depends on Chromium's File System Access permission. With persistent permission the reader restores and resumes watching automatically; otherwise the browser shows one Restore click, and clearing site data requires choosing the file again. A plain drag-drop fallback is a static snapshot. WAL-mode databases are shown read-only but cannot be submitted because a single-file handle cannot prove the latest contents of `encounters.db-wal`.
