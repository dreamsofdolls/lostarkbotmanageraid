...
- **Description (2 dòng)**: line 1 = global summary `<pending>/<eligible> pending (<pct>%) · iLvl <X>` (bỏ dấu ≥, threshold ngầm hiểu; 3-state icon counts đã chuyển xuống footer), line 2 = current page's roster header `📁 <accountName> (<displayName>) · N pending · 🔄<relative>`. Char cards bắt đầu sát dưới description - không có wasted spacer row.
...
- **Footer legend với counts**: `🟢 N done · 🟡 M partial · ⚪ K pending` - icon + count + English label merged per-state. Dynamic per scan (counts thay đổi) nên compute inline thay vì reuse `/raid-status`'s static `STATUS_FOOTER_LEGEND`. Counts di chuyển từ description xuống footer để description gọn hơn, footer rich info hơn.
