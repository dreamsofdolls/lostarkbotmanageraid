...
- **Freshness + countdown line mỗi account embed**: dưới summary line có thêm `📥 Last updated Nh ago · ⏳ Next refresh in Xh · 🔄 Last synced Nm ago · ⏳ Next sync in Ym`. Khi cooldown đã hết show `✅ Refresh ready` / `✅ Sync ready` - user biết ngay chạy lại `/raid-status` sẽ thực sự fetch data mới (không phải hit cache) thay vì đoán mò tại sao iLvl hoặc raid progress chưa update. `📥` = roster metadata gate 2h (always visible). `🔄` = auto-manage bible log sync gate 5m (chỉ hiện khi user opt-in).
