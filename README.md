...
- **Per-user+roster grouping**: section header format `👤 <displayName> · 📁 <accountName> · N pending · ⚡ X partial · 🔄 auto`. User có 2 roster (main + alt) hiện 2 section riêng biệt với accountName phân biệt. Rosters cùng 1 user group consecutive, sort by user-total pending desc rồi per-roster pending desc rồi alphabetical tie-break.
- Field value có `_synced <relative-time>_` prepend khi opted-in user có sync data → Raid Manager biết data fresh hay stale per-user.
