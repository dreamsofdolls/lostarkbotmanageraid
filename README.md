...
- Raid signup board preview (`/raid-schedule-preview`): Support/DPS slots, waitlist, RSVP, auto-lock, a lead Manage menu (incl. placement-aware add-member ping + member kick with auto waitlist-promotion), multi-turn (bus) team assignment, a `📊 Turn plan` button (read-only peek), and `show` to resurface a buried board to the channel bottom (with a `🗓 board switcher` when the lead runs several); party size is derived from the selected raid (Act 4/Kazeros = 8, Serca = 4)
