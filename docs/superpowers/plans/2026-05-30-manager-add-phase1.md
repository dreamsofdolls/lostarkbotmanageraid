...
**Architecture:** Additive on the shipped self-service board. New lead-only Manage button `➕ Thêm người` opens a native Discord User Select (`rse:adduser:<eventId>`); selecting a user loads their roster and shows a character String Select (`rse:addpick:<targetId>:<eventId>`); selecting a character writes the signup on the target's behalf by reusing the existing pure `applyJoin`, re-renders the board, and pings the user publicly. Works even when the board is locked (manager-add bypasses lock). Placement follows normal Support/DPS capacity: free role slot = comp, full role = waitlist.
...
```javascript
      ...
      addedPlacementWaitlist: "đang ở hàng chờ vì slot theo role đã đầy",
...
```javascript
      ...
      addedPlacementWaitlist: "on the waitlist because that role is full",
...
```javascript
      ...
      addedPlacementWaitlist: "その role の枠が埋まっているので待機列",
...
- Consent = normal signup, auto-clear uniform: reuses `applyJoin` (same signup shape), no special flag; only actual comp slot-holders are auto-cleared. ✓
